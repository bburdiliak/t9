...
const toLetters = split("");

const toPath = (letters: string[]): string =>
  flatten(zip<string, string>(
    [...Array(letters.length).keys()].map(() => "children"),
    letters
  )).join(".");

const toChildrenPath = (letters: string[]): string => {
  if (letters.length === 0) { return ""; }
  if (letters.length === 1) { return "children"; }
  return `${ toPath(letters.slice(0, letters.length - 1)) }.children`;
};

const allPaths = (word: string): string[] =>
  reduce(
    (acc: string[], letter: string) => {
      const nextPath = `${acc.length > 0 ? acc[acc.length - 1] : ""}${letter}`;
      return append(nextPath, acc);
    },
    [],
    toLetters(word)
  );

const buildTree = (words: string[]) => {
  ...
  const reduced = reduce(
    (tree, word) => {
      const isPrefixOfExisting = compose(toChildrenPath, toLetters)(word);
      if (hasPath(isPrefixOfExisting, tree)) {
        return change(tree, toPath(toLetters(word)).concat(".contained"), true);
      }

      const treeWithAllKeys = reduce(
        (growingTree, wordFraction) => {
          const letters = toLetters(wordFraction);
          const childrenPath = toChildrenPath(letters);
          const specificChildPath = toPath(letters);

          const treeWithChildren = (!hasPath(childrenPath, growingTree)) ?
            change(growingTree, childrenPath, {  }) :
            growingTree;

          const treeWithSpecificChild = (!hasPath(specificChildPath, treeWithChildren)) ?
            change(treeWithChildren, specificChildPath, { contained: false }) :
            treeWithChildren;

          return treeWithSpecificChild;
        },
        tree,
        allPaths(word)
      );

      return change(treeWithAllKeys, toPath(toLetters(word)).concat(".contained"), true);
    },
...
const memoizedTree = memoize<(words: string[]) => Node>(buildTree);
const commonWordsTree = () => memoizedTree(commonWords);

const isReal = (word: string): boolean => {
  const realWords = commonWordsTree();
  const path = `${toPath(split("", word))}.contained`;
  return hasPath(path, realWords) && pathEq(path, true, realWords);
};
